...
namespace WpfInvestigate.Controls
{
    public partial class MwiItem
    {
        ...
        {
            if (MwiItems != null && (Position.X < 0 || Position.Y < 0))
                Position = MwiItems.GetStartPositionForItem(this);
        ...
        private async Task AnimateWindowState(WindowState previousWindowState)
        {
            ...
            if (WindowState == WindowState.Minimized)
            {
                ...
                {
                    var newMwiItem = MwiItems.InternalWindows.Any(w => w.WindowState != WindowState.Minimized)
                        ? MwiItems.InternalWindows.Where(w => w.WindowState != WindowState.Minimized)
