...
{
  public class GlDocLineOld
  {

    private static CultureInfo ci = CultureInfo.InvariantCulture;
    private static int cnt = 0;
    public static IDictionary<int, GlDocLineOld> Data;

    public static IEnumerable<GlDocLineOld> ReadData(int records = 100)
    {
      ...
      using (var cmd = conn.CreateCommand())
      {
        ...
        using (var rdr = cmd.ExecuteReader())
        {
          ...
          var oo = new object[columnCount];
          while (rdr.Read())
          {
            rdr.GetValues(oo);
            yield return new GlDocLineOld(oo);
          }
        }
      }
    }

    public static void RefreshDataParallel(int records = 100)
    {
      ...
      Data = new Dictionary<int, GlDocLineOld>();
      var sw = new Stopwatch();
      sw.Start();

      // Data = ReadData(records).AsParallel().ToDictionary(x => x._ID_, x => x);
      Data = ReadData(records).ToDictionary(x => x._ID_, x => x);
    ...
    public static void RefreshData(int records = 100)
    {
      ...
      // Data = new Dictionary<int, GlDocLineOld>(3000010);
      Data = new Dictionary<int, GlDocLineOld>();
    ...
    public MastCoA oACCOUNT2 => MastCoA.Data[ACCOUNT];
    ...
    public GlDocLineOld(DbDataReader r)
    {
      DOCKEY = r.GetInt64(0);
      LINENO = r.GetInt16(1);
